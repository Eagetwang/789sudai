...
$modelLabel = new \backend\models\FrontWebsiteCount();
?>
        ...
        <div class="box-body">
          <div id="example1_wrapper" class="dataTables_wrapper form-inline dt-bootstrap">
            <!-- row start search-->
          	<div class="row">
          	<div class="col-sm-12">
                <?php ActiveForm::begin(['id' => 'front-website-count-search-form', 'method'=>'get', 'options' => ['class' => 'form-inline'], 'action'=>Url::toRoute('front-website-count/index')]); ?>     
...
<!--                      <input type="text" class="form-control" id="query[type]" name="query[type]"  value="--><?//=isset($query["type"]) ? $query["type"] : "" ?><!--">-->
            ...
            <?php 
              ...
              echo '<th onclick="orderby(\'look_total\', \'desc\')" '.CommonFun::sortClass($orderby, 'look_total').' tabindex="0" aria-controls="data_table" rowspan="1" colspan="1" aria-sort="ascending" >'.$modelLabel->getAttributeLabel('look_total').'</th>';
	...
	<div class="modal-dialog">
		<div class="modal-content">
			...
			<div class="modal-body">
                <?php $form = ActiveForm::begin(["id" => "front-website-count-form", "class"=>"form-horizontal", "action"=>Url::toRoute("front-website-count/save")]); ?>                      
          ...
          <div id="date_div" class="form-group">
              ...
              <div class="col-sm-10">
                  <input type="text" class="form-control" id="date" name="FrontWebsiteCount[date]" placeholder="必填" />
              </div>
          ...
          <div id="type_div" class="form-group">
              ...
              <div class="col-sm-10">
                  <input type="text" class="form-control" id="type" name="FrontWebsiteCount[type]" placeholder="必填" />
              </div>
              <div class="clearfix"></div>
          </div>
          ...
          <div id="pv_div" class="form-group">
              ...
              <div class="col-sm-10">
                  <input type="text" class="form-control" id="pv" name="FrontWebsiteCount[pv]" placeholder="必填" />
              </div>
              <div class="clearfix"></div>
          </div>
          ...
          <div id="uv_div" class="form-group">
              ...
              <div class="col-sm-10">
                  <input type="text" class="form-control" id="uv" name="FrontWebsiteCount[uv]" placeholder="必填" />
              </div>
              <div class="clearfix"></div>
          </div>
          ...
          <div id="register_total_div" class="form-group">
              ...
              <div class="col-sm-10">
                  <input type="text" class="form-control" id="register_total" name="FrontWebsiteCount[register_total]" placeholder="必填" />
              </div>
              <div class="clearfix"></div>
          </div>

          <div id="look_total_div" class="form-group">
              <label for="look_total" class="col-sm-2 control-label"><?php echo $modelLabel->getAttributeLabel("look_total")?></label>
              <div class="col-sm-10">
                  <input type="text" class="form-control" id="look_total" name="FrontWebsiteCount[look_total]" placeholder="必填" />
              </div>
              <div class="clearfix"></div>
          </div>
          ...
          <div id="apply_total_div" class="form-group">
              ...
              <div class="col-sm-10">
                  <input type="text" class="form-control" id="apply_total" name="FrontWebsiteCount[apply_total]" placeholder="必填" />
              </div>
              <div class="clearfix"></div>
          </div>
...
function orderby(field, op){
 ...
 function searchAction(){
		$('#front-website-count-search-form').submit();
	}
 ...
 function initEditSystemModule(data, type){
	if(type == 'create'){
    	...
    	$("#look_total").val(data.look_total);
	...
	if(type == "view"){
      ...
      $("#look_total").attr({readonly:true,disabled:true});
	...
	$('#edit_dialog_ok').addClass('hidden');
      ...
      $("#look_total").attr({readonly:false,disabled:false});
...
function initModel(id, type, fun){
	...
	$.ajax({
		   type: "GET",
		   url: "<?=Url::toRoute('front-website-count/view')?>",
		   data: {"id":id},
		   cache: false,
		   dataType:"json",
...
function deleteAction(id){
	...
	if(ids.length > 0){
		admin_tool.confirm('请确认是否删除', function(){
		    $.ajax({
				   type: "GET",
				   url: "<?=Url::toRoute('front-website-count/delete')?>",
				   data: {"ids":ids},
				   cache: false,
				   dataType:"json",
				   error: function (xmlHttpRequest, textStatus, errorThrown) {
...
$('#edit_dialog_ok').click(function (e) {
    e.preventDefault();
	$('#front-website-count-form').submit();
});
...
$('#front-website-count-form').bind('submit', function(e) {
	e.preventDefault();
	var id = $("#id").val();
	var action = id == "" ? "<?=Url::toRoute('front-website-count/create')?>" : "<?=Url::toRoute('front-website-count/update')?>";
    $(this).ajaxSubmit({
    	type: "post",
    	dataType:"json",
    	url: action,
